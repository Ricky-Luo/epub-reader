...
var _render = function(tree, checkbox, isSelectableGroups, chain, depth) {
    depth = depth || 0;
    var i = 0, name = chain, has_subitems, expanded;
    var tpl = '<ul ';
    if (depth) {
        tpl += 'class="ui-tree-subitems';
        chain += '.subitems';
    } else {
    ...
    for (; i < tree.length; i+=1) {
        has_subitems = tree[i].subitems && tree[i].subitems.length;
        name = chain + '[' + i + ']';
        expanded = has_subitems ? 'data-ui-tree-expanded="' + ((!depth && tree.length === 1) || !!tree[i].expanded) + '"' : '';
        tpl += '<li data-id="'+tree[i].id+'" data-label="'+tree[i].label+'" data-href="'+tree[i].href+'" class="ui-tree-item' + (has_subitems ? ' ui-tree-item-group' : '') + '" ' + expanded + '>';
        tpl += '<div class="ui-tree-item-label"><label class="ui-tree-item-label-real">';
        // tpl += '<span class="ui-tree-item-toggler"></span>';
        tpl += '<span class="ui-tree-item-text">' + tree[i].label + '</span>';
        tpl += '</label></div>';
        if (has_subitems) {
            tpl += _render(tree[i].subitems, checkbox, isSelectableGroups, name, depth+1);
        }
        tpl += '</li>';
    }
    tpl += '</ul>';
